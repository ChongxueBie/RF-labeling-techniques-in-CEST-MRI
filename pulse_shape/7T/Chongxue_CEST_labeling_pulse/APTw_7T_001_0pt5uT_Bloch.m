...
seq_defs.td            = 0.005            ; % interpulse delay [s]
...
seq_defs.offsets_ppm   = [seq_defs.M0_offset -6:0.1:6]; % offset vector [ppm]
...
B0          = seq_defs.B0;          % B0 [T]
B1pa        = 0.512;  % mean sat pulse b1 [uT]
...
for currentOffset = offsets_Hz
    ...
    accumPhase=0;
    for np = 1:n_pulses
%         satPulse.phaseOffset = 0; %mod(accumPhase,2*pi); % set accumulated pahse from previous rf pulse
...
M_z = simulate_pulseqcest(seq_filename,'../../sim-library/cest_labeling_pulse_7T_bmsim.yaml');
...
%% save
% saveDir = '/Volumes/T7/CEST_labeling/data/pulse_shape/3T/';
saveDir = '/Volumes/T7/CEST_labeling/data/pulse_shape/7T/';
% % 
filename=[saveDir,'Z-spectrum_Block.mat'];
% filename=[saveDir,'Z-spectrum_Block_no_Phase.mat'];
% 
save(filename,'M_z');